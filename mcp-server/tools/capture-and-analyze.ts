...
export class CaptureAndAnalyzeTool {
   ...
   */
  async execute(input: CaptureAndAnalyzeInput): Promise<ToolResponse> {
    ...
    try {
      ...
      const integuruAnalysis = await this.analyzeWithInteguru(
        finalNetworkCapture.harFile,
        taskDescription
      );
      
      // Step 8: Choose optimal modality
      const modalityChoice = await this.chooseOptimalModality(
        finalNetworkCapture,
        integuruAnalysis,
        taskDescription
      );
   ...
   */
  private async startNetworkCapture(sessionId: string, captureLevel: number): Promise<NetworkCapture> {
   ...
   */
  private async waitForUserAction(timeoutSeconds: number): Promise<void> {
   ...
   */
  private async stopNetworkCapture(networkCapture: NetworkCapture): Promise<NetworkCapture> {
   ...
   */
  private extractRequestCount(harData: any): number {
    try {
   ...
   */
  private extractDomains(harData: any): string[] {
    try {
   ...
   */
  private extractSize(harData: any): number {
    try {
      return harData.log?.entries?.reduce((total: number, entry: any) => {
