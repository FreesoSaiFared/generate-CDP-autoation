...
export class ReplayAutomationTool {
   ...
   */
  async execute(input: ReplayAutomationInput): Promise<ToolResponse> {
    ...
    try {
      ...
      const actionsToReplay = this.prepareActionsForReplay(
        recording.actions, 
        actionIndex, 
        speedMultiplier
      );
      ...
      const replayResult = await this.executeReplay(
        page, 
        actionsToReplay, 
        skipScreenshots, 
        dryRun
   ...
   */
  private async loadSessionRecording(sessionId: string): Promise<SessionRecording | null> {
    try {
   ...
   */
  private validateRecording(recording: SessionRecording): void {
  ...
  private prepareActionsForReplay(
    actions: SessionRecording['actions'],
   ...
   */
  private async launchReplayBrowser(dryRun: boolean): Promise<puppeteer.Browser> {
  ...
  private async executeAction(
    page: puppeteer.Page,
    action: SessionRecording['actions'][0],
    skipScreenshots: boolean,
    screenshots: string[]
  ): Promise<void> {
    switch (action.type) {
   ...
   * Execute CDP command
   */
  private async executeCDPCommand(page: puppeteer.Page, method: string, params: any): Promise<void> {
    try {
   ...
   */
  private async captureReplayScreenshot(page: puppeteer.Page, screenshots: string[]): Promise<void> {
    try {
   ...
   */
  async getAvailableSessions(): Promise<Array<{ sessionId: string; task: string; date: Date }>> {
    try {
      const sessionsDir = this.config.sessions.storageDir;
      const files = await (fs as any).readdir(sessionsDir);
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          try {
            const filePath = (path as any).join(sessionsDir, file);
            const stats = await (fs as any).stat(filePath);
            const recording = await fs.readJson(filePath);
            
            sessions.push({
              sessionId: (path as any).basename(file, '.json'),
